...
union all
select 'Cache Effectiveness', (round(blks_hit * 100::numeric / nullif(blks_hit + blks_read, 0), 2))::text || '%' from data
union all
select 'Successful Commits', (round(xact_commit * 100::numeric / nullif(xact_commit + xact_rollback, 0), 2))::text || '%' from data
union all
