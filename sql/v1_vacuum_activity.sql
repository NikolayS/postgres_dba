...
-- In PG17+, pg_stat_progress_vacuum renamed:
--   num_dead_tuples -> num_dead_item_ids
--   max_dead_tuples -> max_dead_tuple_bytes (semantics changed: count -> bytes)
...
select
  ...
  waiting as "Wait",
  phase as "Phase",
  scanned || ' (' || coalesce(scanned_pct::text, '?') || '%)' || e' scanned\n'
    || vacuumed || ' (' || coalesce(vacuumed_pct::text, '?') || '%) vacuumed' as "Heap Vacuuming",
  index_vacuum_count || ' completed cycles,'
    || e'\n'
    || case
      when num_dead_item_ids > 10^12 then round(num_dead_item_ids::numeric / 10^12::numeric, 0)::text || 'T'
      when num_dead_item_ids > 10^9 then round(num_dead_item_ids::numeric / 10^9::numeric, 0)::text || 'B'
      when num_dead_item_ids > 10^6 then round(num_dead_item_ids::numeric / 10^6::numeric, 0)::text || 'M'
      when num_dead_item_ids > 10^3 then round(num_dead_item_ids::numeric / 10^3::numeric, 0)::text || 'k'
      else num_dead_item_ids::text
    end
    || e' dead item IDs collected\n'
    || '(mem limit: ' || dead_tuple_mem || ')' as "Index Vacuuming"
from data
...
select
  ...
  index_vacuum_count || ' completed cycles,'
    ...
    || ' (' || coalesce(dead_pct::text, '?') || e'%) dead tuples\nof max ~'
    || case
